...
def noise_calculator(cube, hdr_map, in_par):
    ...
    if in_par['CONT_COR']:
        ...
        cube = cube - zerop
    # we split the cube in two parts, a half with the emission line and a half
    # without
    ind_max = cube.argmax(axis=0)
    ind_out = ind_max - zlen / 2
    ind_aux = ind_max + zlen / 2
    ind_out[numpy.where(ind_out < 0)] = ind_aux[numpy.where(ind_out < 0)]
    mask_line = ((abs(index_cube - ind_max) < zlen / 4) |
                 (abs(abs(index_cube - ind_max) - zlen) < zlen / 4))
    mask_out = ((abs(index_cube - ind_out) < zlen / 4) |
                (abs(abs(index_cube - ind_out) - zlen) < zlen / 4))
    flux_line = numpy.mean((cube) * mask_line, 0)
    flux_out = numpy.mean((cube) * mask_out, 0)
    # we demand a minimum difference in flux between on-line and off-line
    mask_ratio = flux_line / flux_out > in_par['FLUX_RATIO']

    std = numpy.std(cube, 0)
    line_noise = numpy.median(
        std[numpy.where((abs(flux_line / flux_out - 1) < in_par['NOISE_PERC']))])
    ...
    mask_max = (maxima > in_par['MAX_TO_NOISE'] * line_noise)
...
# center the line profile, only valid for instruments with a cyclical
# range like Fabry-Perots
def line_centering(vel, perf):
    ...
    if max_ch < sizez / 2:
        ...
        vel2[
            sizez - l1:sizez] = numpy.arange(1, l1 + 1) * v_res + vel[sizez - 1]
        perf2 = numpy.array([perf2])[0]
        vel2 = numpy.array([vel2])[0]
    else:
        perf2 = perf
...
def get_pars(profile_i, num_zeros, v_res):
    ...
    for zeros in range(num_zeros / 2):
        ...
        params.append(
            0.5 * (profile_i.zero[2 * zeros] + profile_i.zero[2 * zeros + 1]))
        params.append(
            0.5 * (profile_i.zero[2 * zeros + 1] - profile_i.zero[2 * zeros]))
    ...
    for index in range(num_zeros / 2):
        ...
        # minimum velocity is first zero of the second derivative
        minpars[3 * index + 1] = profile_i.zero[2 * index]
        # minimum dispersion is half the distance between zeros
        minpars[3 * index + 2] = 0.5 * params[3 * index + 2]

        # maximum intensity is 1.5 times the estimated value
        maxpars[3 * index] = 1.5 * params[3 * index]
        # maximum velocity is the second zero of the second derivative
        maxpars[3 * index + 1] = profile_i.zero[2 * index + 1]
        # maximum dispersion is double the distance between zeros
        maxpars[3 * index + 2] = 2. * params[3 * index + 2]

        # if the estimated velocity dispersion is below the resolution we force
        # values based on it
        if params[3 * index + 2] < v_res:
...
# add the main peak parameters for the second pass, and if it has been
# detected, correct them
def fix_primpars(params, minpars, maxpars, fit_params, num_zeros2, ind_max, v_res):
    vels = [params[index] for index in numpy.arange(1, len(params), 3)]
    prin_index = min(list(range(len(vels))), key=lambda k: abs(
        vels[k] - fit_params[3 * ind_max + 1]))
    if abs(vels[prin_index] - fit_params[3 * ind_max + 1]) < 2. * v_res:
        num_zeros = num_zeros2
        # we impose the initial parameters of the main peak as the result of
        # the previous fit
        params[3 * prin_index:3 * prin_index +
               3] = fit_params[3 * ind_max:3 * ind_max + 3]
        ...
        prin_pars = numpy.array(
            [fit_params[3 * ind_max], fit_params[3 * ind_max + 1], fit_params[3 * ind_max + 2]])
        prin_minpars = numpy.array([0.8 * fit_params[3 * ind_max], fit_params[
                                   3 * ind_max + 1] - 2 * v_res, 0.7 * fit_params[3 * ind_max + 2]])
        prin_maxpars = numpy.array([1.5 * fit_params[3 * ind_max], fit_params[
                                   3 * ind_max + 1] + 2 * v_res, 1.3 * fit_params[3 * ind_max + 2]])
...
class profile(object):  # class that contains the information and operations related to the line profile Ginsburg
    ...
    def get_zeros(self, rms2, interp):
        # number of elements of the interpolated profile
        n_elements = len(self.velocity) * interp
        ...
        perfil_fino = numpy.interp(
            velocity_fino, self.velocity, self.intensity)
        ...
        for i in range(n_elements - 1):
            ...
            if signo < 0:
                ...
                if not((self.cont == 0) and (deriv2[i] < deriv2[i + 1])):
                    # we keep the zero only if it goes from negative to
                    # positive, as otherwise it corresponds to a different,
                    # undetected peak
                    if ((((self.cont + 1) % 2) == 0) and (self.cont != 0)):
                        # busca la velocidad del anterior cero
                        velant = self.zero[-1]
                        ind_antvel = numpy.where(numpy.array(velocity_fino) == velant)[
                            0]  # busca el indice del perfil correspondiente
                        # we estimate the intensity as the highest point
                        # between the zeros
                        flux_max = max(perfil_fino[ind_antvel:i])
                        if (max(perfil_fino[ind[self.cont - 1]:i]) >= rms2):

                            # we accept the peak if it is higher thatn twice
                            # the rms
                            ...
                            # if not, we do not consider it and we must erase
                            # the previous zero
                            self.cont = self.cont - 1
    ...
    # wrapper for the fitting algorithm
    def gaussianfit(self, num_zeros, params, limitedmin, limitedmax, minpars, maxpars):

        ajuste = multigaussfit(self.velocity, self.intensity, ngauss=num_zeros / 2, params=params,
                               limitedmin=limitedmin, limitedmax=limitedmax, minpars=minpars, maxpars=maxpars, quiet=True, shh=True)
        fit_params = ajuste[0][:]
        return fit_params
...
# creates the correct header for the bubble maps from the relevant
# parameters of the previous and current run
def header_correct(hdr_prev, hdr_new):
    changekeys = ['RERUN', 'MAIN_OUT', 'MAIN_EXT', 'MAIN_NAME', 'BUB_V_SIGMA', 'BUB_USE_I',
                  'BUB_I_R', 'BUB_USE_D', 'BUB_D_R', 'PROF_OUT', 'PROF_VUNIT', 'PROF_IUNIT']
...
def plot_profile(cube, vel, i, j, centering, vel_map, mono_map, disp_map, in_par):
    ...
    for l in det_p[0]:
        perf_l = numpy.pi**(-0.5) * mono_map[l, i, j] * numpy.exp(-((vel_gauss - vel_map[l, i, j])**2.) / (
            2. * disp_map[l, i, j]**2.)) / (disp_map[l, i, j])  # c es ajustada a 2*c**2
    ...
    # inverted i and j as per Python notation
    plt.title('x=' + str(j) + ', y=' + str(i))
