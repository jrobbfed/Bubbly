...
def moments(data, circle, rotate, vheight, estimator=median, **kwargs):
    ...
    width_x = np.sqrt(np.abs((np.arange(col.size) - y)
                             * col).sum() / np.abs(col).sum())
    row = data[:, int(x)]
    width_y = np.sqrt(np.abs((np.arange(row.size) - x)
                             * row).sum() / np.abs(row).sum())
...
def gaussfit(data, err=None, params=(), autoderiv=True, return_all=False, circle=False,
    ...
    if usemoment.any() and len(params) == len(usemoment):
        moment = np.array(moments(data, circle, rotate,
                                  vheight, **kwargs), dtype='float')
    ...
    # mpfit will fail if it is given a start parameter outside the allowed
    # range:
    for i in range(len(params)):
        if params[i] > maxpars[i] and limitedmax[i]:
            params[i] = maxpars[i]
        if params[i] < minpars[i] and limitedmin[i]:
            params[i] = minpars[i]
    ...
    parinfo = [
        {'n': 1, 'value': params[1], 'limits':[minpars[1], maxpars[1]], 'limited':[
            limitedmin[1], limitedmax[1]], 'fixed':fixed[1], 'parname':"AMPLITUDE", 'error':0},
        {'n': 2, 'value': params[2], 'limits':[minpars[2], maxpars[2]], 'limited':[
            limitedmin[2], limitedmax[2]], 'fixed':fixed[2], 'parname':"XSHIFT", 'error':0},
        {'n': 3, 'value': params[3], 'limits':[minpars[3], maxpars[3]], 'limited':[
            limitedmin[3], limitedmax[3]], 'fixed':fixed[3], 'parname':"YSHIFT", 'error':0},
        {'n': 4, 'value': params[4], 'limits':[minpars[4], maxpars[4]], 'limited':[limitedmin[4], limitedmax[4]], 'fixed':fixed[4], 'parname':"XWIDTH", 'error':0}]
    if vheight == 1:
        parinfo.insert(0, {'n': 0, 'value': params[0], 'limits': [minpars[0], maxpars[0]], 'limited': [
                       limitedmin[0], limitedmax[0]], 'fixed': fixed[0], 'parname': "HEIGHT", 'error': 0})
    if circle == 0:
        parinfo.append({'n': 5, 'value': params[5], 'limits': [minpars[5], maxpars[5]], 'limited': [
                       limitedmin[5], limitedmax[5]], 'fixed': fixed[5], 'parname': "YWIDTH", 'error': 0})
        if rotate == 1:
            parinfo.append({'n': 6, 'value': params[6], 'limits': [minpars[6], maxpars[6]], 'limited': [
                           limitedmin[6], limitedmax[6]], 'fixed': fixed[6], 'parname': "ROTATION", 'error': 0})
...
def onedmoments(Xax, data, vheight=True, estimator=median, negamp=None,
    ...
    # try to figure out whether pos or neg based on the minimum width of the
    # pos/neg peaks
    Lpeakintegral = integral - height * \
        len(Xax) * dx - (data[data > height] * dx).sum()
    Lamplitude = data.min() - height
    Lwidth_x = 0.5 * (np.abs(Lpeakintegral / Lamplitude))
    Hpeakintegral = integral - height * \
        len(Xax) * dx - (data[data < height] * dx).sum()
    ...
    if negamp:  # can force the guess to be negative
        ...
        if Hstddev < Lstddev:
            xcen, amplitude, width_x, = Xax[
                np.argmax(data)], Hamplitude, Hwidth_x
        else:
            xcen, amplitude, width_x, = Xax[
                np.argmin(data)], Lamplitude, Lwidth_x
...
def onedgaussfit(xax, data, err=None,
    ...
    if vheight is False:
        ...
        fixed[0] = True
    if usemoments:
        params = onedmoments(xax, data, vheight=vheight,
                             negamp=negamp, veryverbose=veryverbose)
        if vheight is False:
    ...
    parinfo = [{'n': 0, 'value': params[0], 'limits':[minpars[0], maxpars[0]], 'limited':[limitedmin[0], limitedmax[0]], 'fixed':fixed[0], 'parname':"HEIGHT", 'error':0},
               {'n': 1, 'value': params[1], 'limits':[minpars[1], maxpars[1]], 'limited':[
                   limitedmin[1], limitedmax[1]], 'fixed':fixed[1], 'parname':"AMPLITUDE", 'error':0},
               {'n': 2, 'value': params[2], 'limits':[minpars[2], maxpars[2]], 'limited':[
                   limitedmin[2], limitedmax[2]], 'fixed':fixed[2], 'parname':"SHIFT", 'error':0},
    ...
    if (not shh) or veryverbose:
        ...
        print("Chi2: ", mp.fnorm, " Reduced Chi2: ", mp.fnorm /
              len(data), " DOF:", len(data) - len(mpp))
...
def collapse_gaussfit(cube, xax=None, axis=2, negamp=False, usemoments=True, nsigcut=1.0, mppsigcut=1.0,
    ...
    print("Fitting a total of %i spectra with peak signal above %f" % (
        (np.abs(extremum(cube, axis=0)) > (mean_std * nsigcut)).sum(), mean_std * nsigcut))
    for i in range(cube.shape[1]):
        t0 = time.time()
        nspec = (np.abs(extremum(cube[:, i, :], axis=0)) > (
            mean_std * nsigcut)).sum()
        print("Working on row %d with %d spectra to fit" % (i, nspec), end=' ')
        for j in range(cube.shape[2]):
            if np.abs(extremum(cube[:, i, j])) > (mean_std * nsigcut):
                mpp, gfit, mpperr, chi2 = onedgaussfit(xax, cube[:, i, j], err=np.ones(
                    cube.shape[0]) * mean_std, negamp=negamp, usemoments=usemoments, **kwargs)
