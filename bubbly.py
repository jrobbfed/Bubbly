...
# proceed to multi-peak detection and fitting unless the code is rerun on
# the same multi maps:
if not in_par['RERUN']:
    print('performing peak detectiong and fitting')
    # Initialize dictionaries and lists where we will store data for the
    # calculations
        ...
        for j in range(sizey):
            if mask[i, j]:
                ...
                if num_zeros > 1:
                    ...
                    # we impose a higher lower limit to the main peak to ensure
                    # it is the most intense after fitting
                    ind_max = numpy.argmax(
                        params[numpy.arange(0, len(params), 3)])
                    minpars[3 * ind_max] = 0.9 * params[3 * ind_max]
                    limitedmin = [True, True, True]
                    limitedmax = [True, True, True]
                    # we perform the first fit, which we will use to subtract
                    # the main peak and then perform the peak detection again
                    fit_params = profile_i.gaussianfit(num_zeros, list(
                        params), limitedmin, limitedmax, list(minpars), list(maxpars))
                    # we redo the peak detection subtracting the main peak
                    perf_2 = perf_p - fit_params[3 * ind_max] * numpy.exp(-(
                        (vel_p - fit_params[3 * ind_max + 1])**2.) / (2. * fit_params[3 * ind_max + 2]**2.))
                    profile_i = profile(vel_p, perf_2)
                    num_zeros2 = profile_i.get_zeros(SNR_S * spec_noise, 10)
                    if num_zeros2 > 1:
                        params, minpars, maxpars = get_pars(
                            profile_i, num_zeros2, v_res)
                        params, minpars, maxpars, num_zeros = fix_primpars(
                            params, minpars, maxpars, fit_params, num_zeros2, ind_max, v_res)
                    ...
                    for zeros in range(num_zeros / 2):
                        ...
                        mono_map[zeros, i, j] = fit_params[
                            zeros * 3] * fit_params[zeros * 3 + 2]

    map_number = vel_map.shape[0]
    # We calibrate in intensity, since mono is already int*disp, the
    # calibration is (a*I+b)*c*D=a*c*I*D+b*c*D=a*c*MONO+b*c*D
    mono_map = (pi**0.5) * (mono_map * in_par['CALIB_A'] * in_par[
        'CALIB_C'] + in_par['CALIB_B'] * in_par['CALIB_C'] * disp_map)
...
if in_par['RERUN']:
    ...
    # make correct header from parameters for the multiple maps and the
    # current parfile
    hdr_map = header_correct(hdr_prev, hdr_map)


print('maximum number of peaks: ', map_number)


# We start to construct the main peak maps, we begin by assigning the
# values of where there is only one peak detected
...
veloff = numpy.array([numpy.subtract(vel_map[k, :, :], main_v)
                      for k in numpy.arange(map_number)])
veloff_abs = abs(veloff)
bub = []
# we look for matches to the conditions for expansion presence
for m1 in range(map_number - 1):
    for m2 in range(m1 + 1, map_number):
        bub_m = numpy.where((abs(veloff_abs[m1, :, :] - veloff_abs[m2, :, :]) < in_par['BUB_V_SIGMA'] * v_res) & (veloff_abs[m1, :, :] - veloff_abs[m2, :, :] != 0.) & (veloff_abs[
                            m1, :, :] != 0.) & (veloff_abs[m2, :, :] != 0.) & ((veloff[m1, :, :] < 0) != (veloff[m2, :, :] < 0)) & (veloff_abs[m1, :, :] != abs(main_v)) & (veloff_abs[m2, :, :] != abs(main_v)))
        bub_m = [(bub_m[0][i1], bub_m[1][i1], m1, m2)
                 for i1 in range(len(bub_m[0]))]
        bub.extend(bub_m)
...
if in_par['BUB_USE_I'] == 1:
    ...
    for m1 in range(map_number - 1):
        for m2 in range(m1 + 1, map_number):
            bub_m_i = numpy.where((abs(mono_map[m1, :, :] - mono_map[m2, :, :])) / (
                mono_map[m1, :, :] + mono_map[m2, :, :]) < in_par['BUB_I_R'])
            bub_m_i = [(bub_m_i[0][i1], bub_m_i[1][i1], m1, m2)
                       for i1 in range(len(bub_m_i[0]))]
...
if in_par['BUB_USE_D'] == 1:
    ...
    for m1 in range(map_number - 1):
        for m2 in range(m1 + 1, map_number):
            bub_m_d = numpy.where((abs(disp_map[m1, :, :] - disp_map[m2, :, :])) / (
                disp_map[m1, :, :] + disp_map[m2, :, :]) < in_par['BUB_D_R'])
            bub_m_d = [(bub_m_d[0][i1], bub_m_d[1][i1], m1, m2)
                       for i1 in range(len(bub_m_d[0]))]
            bub_d.extend(bub_m_d)
    bub = list(set(bub) & set(bub_d))

# we create the arrays that will contain the information about the
# detected expansions
...
for coord in bub:
    ...
    if in_par['INT_TYPE'] == 0:
        ...
        irel = (pi**0.5) * (irel * in_par['CALIB_A'] * in_par['CALIB_C'] + in_par[
            'CALIB_B'] * in_par['CALIB_C'] * disp_map[m2, i, j])

    # we check whether there is already a detection at that point and add
    # additional maps if necessary
    if numpy.isnan(bubble_map[i, j]):
        ...
        # we select the expansion with the highest velocity to be on the main
        # map
        if voff < bubble_map[i, j]:
            bubsec_index = min(numpy.where(
                numpy.isnan(bubble_sec[:, i, j]))[0])
...
if in_par['PROF_OUT'] == 1:
    ...
    for coord in bub:
        i = coord[0]
        j = coord[1]
        plot_profile(cube, vel, i, j, in_par[
                     'CENTERING'], vel_map, mono_map, disp_map, in_par)
